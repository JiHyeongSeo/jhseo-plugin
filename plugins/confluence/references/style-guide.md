...
*   역할에 맞는 형태를 **적극적으로** 사용합니다 (예: 사용자는 User 아이콘/Ellipse, DB는 Cylinder 형상 등).
*   **기본 아이콘 규칙:** 서버=Python 아이콘, 메신저=Slack 아이콘, AWS=해당 프로덕트 아이콘. 불확실하면 사용자에게 질문할 것.
*   **외부 URL 아이콘 사용 금지** — draw.io 내장 스텐실 또는 Base64 SVG만 사용
*   **아이콘 라벨 padding:** 아이콘 아래 라벨 텍스트와 그림자 사이에 `spacingTop=8;` (8px) 여백 적용

### 겹침 방지 (최우선 규칙)

> **⚠️ 노드와 엣지는 절대 겹치지 않게 배치합니다.**

*   **노드끼리 겹침 금지**: 모든 노드는 충분한 간격을 두고 배치
*   **엣지가 노드 관통 금지**: 연결선이 중간 노드를 뚫고 지나가지 않도록 우회
*   **엣지끼리 겹침 금지**: 연결선이 같은 경로를 공유하지 않도록 경로 분리
*   같은 면에서 여러 엣지를 뽑을 때: 같은 지점이 아닌, 면을 따라 균등 간격으로 연결 (`exitX/exitY` 사용)
*   **라벨 겹침 금지**: `labelBackgroundColor=#ffffff`로 가독성 확보, 라벨 위치를 수동 조정하여 다른 요소와 겹치지 않게 배치
