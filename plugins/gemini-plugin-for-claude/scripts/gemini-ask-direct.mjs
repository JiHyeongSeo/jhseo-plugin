...
import { DEFAULT_MODEL, resolveModel, getDefaultModel } from "./lib/models.mjs";
...
async function main() {
  ...
  const savedDefault = getDefaultModel(workspaceRoot);
  let model = savedDefault || DEFAULT_MODEL;
  const promptParts = [];

  for (let i = 0; i < args.length; i++) {
    if ((args[i] === "--model" || args[i] === "-m") && i + 1 < args.length) {
      model = resolveModel(args[++i]) || savedDefault || DEFAULT_MODEL;
    } else {
