...
export function renderModelReport(report) {
  ...
  if (report.action === "set") {
    lines.push(`Default model changed to **${report.model}**.`);
    lines.push("");
  } else if (report.action === "reset") {
    lines.push(`Default model reset to hardcoded default (**${report.hardcodedDefault}**).`);
    lines.push("");
  }

  lines.push(`Current default: ${report.currentModel}${report.isCustom ? " (custom)" : " (built-in)"}`);
  lines.push(`Review default: ${report.reviewDefault}`);
  lines.push("");
  lines.push("Available aliases:");
  for (const [alias, fullId] of report.aliases) {
    lines.push(`  ${alias} → ${fullId}`);
  }
  lines.push("");
  lines.push("Override per command: `--model <name>` or `-m <name>`");
