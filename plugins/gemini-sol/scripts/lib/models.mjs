...
const BLOCKED_MODEL_PREFIXES = ["claude"];
...
export function validateModel(model) {
  ...
  if (isBlockedModel(model)) {
    return {
      valid: false,
      message: `"${model}" is a Claude model. You are already in Claude Code — use a Gemini model instead.`
    };
  }
...
export function isBlockedModel(model) {
  if (!model) return false;
  const lower = String(model).trim().toLowerCase();
  return BLOCKED_MODEL_PREFIXES.some((prefix) => lower.startsWith(prefix));
}
...
/**
 * Get the effective default model for the given workspace,
 * falling back to the hardcoded default if no config is saved.
 */
export function getDefaultModel(cwd) {
  const config = getConfig(cwd);
  return config.defaultModel || null;
}

/**
 * Save a new default model for the given workspace.
 * Returns the resolved full model ID.
 */
export function setDefaultModel(cwd, model) {
  ...
  setConfig(cwd, "defaultModel", resolved);
  return resolved;
}

/**
 * Clear the saved default model (revert to hardcoded default).
 */
export function clearDefaultModel(cwd) {
  setConfig(cwd, "defaultModel", null);
}

/**
 * Build current model info for reporting.
 */
export function getCurrentModelInfo(cwd) {
  const config = getConfig(cwd);
  const savedModel = config.defaultModel || null;
  return {
    currentModel: savedModel || DEFAULT_MODEL,
    isCustom: Boolean(savedModel),
    savedModel,
    hardcodedDefault: DEFAULT_MODEL,
    reviewDefault: DEFAULT_REVIEW_MODEL
  };
}
