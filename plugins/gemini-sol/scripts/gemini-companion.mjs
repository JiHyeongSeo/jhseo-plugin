...
import { readStdinIfPiped } from "./lib/shared.mjs";
import { collectReviewContext, ensureGitRepository, resolveReviewTarget } from "./lib/shared.mjs";
import { binaryAvailable, terminateProcessTree } from "./lib/shared.mjs";
import { loadPromptTemplate, interpolateTemplate } from "./lib/shared.mjs";
import {
  generateJobId,
  getConfig,
  listJobs,
...
import { resolveWorkspaceRoot } from "./lib/shared.mjs";
import {
...
function printUsage() {
  console.log(
    [
      ...
      "  node scripts/gemini-companion.mjs model [model-name|--reset] [--json]",
...
async function buildSetupReport(cwd, actionsTaken = []) {
  ...
  if (geminiStatus.available && !authStatus.loggedIn) {
    nextSteps.push("Run `!gemini auth login` to authenticate with Google.");
  }
...
function handleModel(argv) {
  const { options, positionals } = parseCommandInput(argv, {
    valueOptions: ["cwd"],
    booleanOptions: ["json", "reset"]
  });

  const cwd = resolveCommandCwd(options);
  const workspaceRoot = resolveCommandWorkspace(options);
  const modelArg = positionals.join(" ").trim();

  if (options.reset) {
    clearDefaultModel(workspaceRoot);
    const info = getCurrentModelInfo(workspaceRoot);
    const payload = {
      action: "reset",
      ...info,
      aliases: [...MODEL_ALIASES]
    };
    outputResult(options.json ? payload : renderModelReport(payload), options.json);
    return;
  }

  if (modelArg) {
    const resolved = setDefaultModel(workspaceRoot, modelArg);
    const info = getCurrentModelInfo(workspaceRoot);
    const payload = {
      action: "set",
      model: resolved,
...
async function executeReviewRun(request) {
  ...
  // Gemini has no native reviewer — always use adversarial review path.
  ...
  await runForegroundCommand(
    ...
    (progress) =>
      executeReviewRun({
        ...
        base: options.base,
        scope: options.scope,
        model: options.model || getDefaultModel(workspaceRoot),
        focusText,
...
async function handleReview(argv) {
  // Gemini has no native reviewer — always use adversarial review path.
  // Unlike Codex, we do NOT call validateNativeReviewRequest here.
...
async function handleTask(argv) {
  ...
  const cwd = resolveCommandCwd(options);
  const workspaceRoot = resolveCommandWorkspace(options);
  const model = normalizeRequestedModel(options.model) ?? getDefaultModel(workspaceRoot);
