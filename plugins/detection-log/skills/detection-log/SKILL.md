---
name: detection-log
description: 텍스트/이미지 탐지 API 로그 검색 및 통계 분석. "탐지 로그", "detection log", "ES 로그", "호출 통계", "탐지율", "서비스별 통계" 등의 키워드에서 활성화
---

# Detection Log 스킬

Elasticsearch에서 텍스트/이미지 탐지 API 호출 로그를 검색하고 통계를 분석하는 스킬입니다.

## 트리거

다음 키워드가 포함된 요청에서 활성화됩니다:
- "탐지 로그", "detection log", "ES 로그", "로그 검색", "로그 조회"
- "호출 통계", "서비스별 통계", "서비스 통계"
- "탐지율", "탐지 통계", "detection rate", "탐지 현황"
- "텍스트 검색" (탐지 로그 맥락에서)
